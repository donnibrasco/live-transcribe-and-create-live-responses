...
# Optional: OpenAI client for generating questions
try:
...
DEVICE_OPTIONS = [13, 12, 1, 0]  # Add your preferred device indices
SAMPLE_RATE = 16000
CHUNK_DURATION = 1
...
def load_transcriber(name: str, backend: str = "auto") -> Transcriber:
    """Load a transcriber using openai-whisper or faster-whisper.

    Prefers openai-whisper when explicitly requested; otherwise falls back to faster-whisper
    which is often easier to install on Windows/Python 3.13.
    """
    # Explicit whisper
...
transcriber = None  # loaded in main after parsing args
...
def find_working_device(preferred_indices=None):
    # Lazy import so dry-run can work without installing sounddevice
    try:
...
def transcribe_stream(device_id, channels, *, silence_threshold=0.001, enqueue_cooldown=5.0, dry_run=False):
    # Lazy imports so --dry-run can run without numpy/scipy/whisper/torch
        ...
        if text and text != last_text:
            ...
            if now - last_enqueue_time >= ENQUEUE_COOLDOWN_SEC:
                ...
                try:
                    ...
                    # Fallback directly if queue fails
...
if __name__ == "__main__":
    ...
    if args.diagnose:
        ...
        # continue; diagnostics printed before run
    if OPENAI_ENABLED:
    ...
    if args.dry_run:
        # Bypass audio + whisper: push a sample transcript and exit
        sample_texts = [
            "Testing pipeline; this should generate a viewer question.",
            "We are checking the AI overlay system in dry-run mode.",
        ]
        for t in sample_texts:
            if not ai_queue.full():
                ai_queue.put_nowait(t)
        ai_queue.join()
        stop_event.set()
